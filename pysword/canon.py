class Passage(tuple):
    def __new__(cls, version, verse_indexes):
        """
        Represents a bible passage, which is a (possibly disjoint) list of
        verses and headings. Sequential access may be able to be optimized.

        verse_indexes is a list of 2-tuples of the form
        (
          (testament, book_index, chapter_index, verse_index),
          (end_testament, end_book_index, end_chapter_index, end_verse_index),
        )

        Note that the ranges are similar to python ranges, so if actual 
        indexes are given for the verse, the last verse will not be included.
        """
        return tuple.__new__(cls, verse_indexes)

    def __init__(self, version, verse_indexes):
        self.version = version

    def __iter__(self):
        """
        Read the verses, and yield them a verse at a time. This will read 
        the data in from the file.
        """
        raise NotImplemented

    def new(self, indexes=None):
        """Returns a new passage that has the same version as this one"""
        if indexes is None:
            return Passage(self.version, self)
        return Passage(self.version, indexes)

    def __getitem__(self, key):
        if isinstance(slice, key):
            return self.new(tuple.__getitem__(self, key))
        else:
            raise NotImplemented

    def __getslice__(self, i, j):
        return self.new(tuple.__getslice__(self, i, j))

    def __len__(self):
        raise NotImplemented

    def __reversed__(self):
        return self.new(tuple.__reversed__(self))

    def __contains__(self, item):
        return NotImplemented

    def __add__(self, other):
        if isinstance(Passage, other) and other.version != self.version:
            raise ValueError('Cannot concatenate different versions')
        return self.new(tuple.__add__(self, other))

    def __mul__(self, other):
        return self.new(tuple.__mul__(self, other))

class Verse(Passage):
    """A Passage that represents a single verse"""
    pass

class Chapter(Passage):
    def __getitem__(self, key):
        if isinstance(slice, key):
            return self.__getslice__(key.start, key.stop)
        elif isinstance(tuple, key):
            if not (1 <= len(key) <= 1):
                raise ValueError
            if len(key) > 1:
                passage = self[key[0]]
            ...
            verse_index = (self[0][0][:2] + (key,), None)
            return Verse(self.version, (verse_index),)

    def __getslice__(self, i, j):
        if isinstance(tuple, i):
            start = self[0][0][3] + i (None,) * (1 - len(i))
        else:
            start = self[0][0][3] + (i, None)
        if isinstance(tuple, j):
            stop = self[0][0][:3] + (j,)
        else:
            stop = self[0][0][:3] + (j,)

class Book(Passage):
    """A Passage that is slicable by chapter"""
    ...
    def __getitem__(self, key):
        if isinstance(slice, key):
            ...
            if not (1 <= len(key) <= 2):
                raise ValueError('Invalid length tuple')
            if len(key) > 1:
                passage = self[key[0]]
            if len(key) > 2:
                passage = passage[key[1]]
            return passage
        else:
            verse_index = ((self[0][0][0], self[0][0][1], key, None), None)
            return Chapter(self.version, (verse_index,))

    def __getslice__(self, i, j):
        if isinstance(tuple, i):
            start = self[0][0][:2] + i + (None,) * (2 - len(i))
        else:
            start = self[0][0][:2] + (i, None)
        if isinstance(tuple, j):
            stop = self[0][0][:2] + j + (None,) * (2 - len(j))
        else:
            stop = self[0][0][:2] + (j, None)

        return Passage(self.version ((start, stop),))

class Testament(Passage):
    """A Passage that is slicable by book"""
    ...
    def __getitem__(self, key):
        if isinstance(slice, key):
            ...
            verse_index = ((self[0][0][0], key, None, None), None)
            return Book(self.version, (verse_index,))
            
    def __getslice__(self, i, j):
        if isinstance(tuple, i):
            start = self[0][0][:1] + i + (None,) * (3 - len(i))
        else:
            start = self[0][0][:1] + (i, None, None)
        if isinstance(tuple, j):
            stop = self[0][0][:1] + j + (None,) * (3 - len(j))
        else:
            stop = self[0][0][:1] + (j, None, None)

        return Passage(self.version, ((start, stop),))

class Bible(Passage):
    """A Passage that is indexable by testament"""
    ...
    def __getitem__(self, key):
        if isinstance(slice, key):
            ...
            if len(key) > 2:
                passage = passage[key[1]]
            if len(key) > 3:
                passage = passage[key[2]]
            return passage
        else:
            verse_index = ((key, None, None, None), None)
            return Testament(self.version, (verse_index,))

    def __getslice__(self, i, j):
        if isinstance(tuple, i):
            start = i + (None,) * (4 - len(i))
        else:
            start = (i, None, None, None)
        if isinstance(tuple, j):
            stop = j + (None,) * (4 - len(j))
        else:
            stop = (j, None, None, None)
...
class Book(object):
    """
    Creates Book with appropriate testament and offset values. If the value 
    for testament is not set, it assumed to be the same as the last time 
    the class was instantiated. Similarly, the base offset is calculated based 
    on the base offset and verses of the previous instantiation.
    """

    _testament = None
    # Headings are Bible?, Testament, Book, Chapter
    _offset = 1 # Start after the Bible heading

    def __init__(self, name, abbr, chap_lens, testament=None):
        if Book.testament != testament:
            Book.offset += 1 # after testament heading
            Book.testament = testament
        Book.offset += 1 # after book heading

        self.testament, self.offset = Book.testament, Book.offset
        self.name, self.abbr, self.chap_lens = name, abbr, chap_lens
    
    def matches(self, input):
        """
        Determine if an input string matches this book.
        Assumes that the caller will be able to properly handle multiple
        matches, since we just see if it starts right.

        Could cause annoying problems if there is a book's full name is the 
        beginning part of another book, like the hypothetical 'John' to
        'John's Revelation', since an input of 'John' would always match both.
        """
        names = (self.name, self.abbr)
        return any(x.lower().startswith(input.lower()) for x in names)
